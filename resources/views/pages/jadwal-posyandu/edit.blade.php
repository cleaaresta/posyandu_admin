        ...
        <div class="w-full max-w-full px-3 shrink-0 md:w-8/12 md:flex-0">
            <div
                ...
                <div class="border-black/12.5 rounded-t-2xl border-b-0 border-solid p-6 pb-0">
                    <h5 class="mb-0 dark:text-white">Formulir Edit: {{ $jadwal->tema }}</h5>
                </div>
                    ...
                    <form action="{{ route('jadwal-posyandu.update', $jadwal->jadwal_id) }}" method="POST"
                            ...
                            <select name="posyandu_id"
                                ...
                                @foreach ($posyandus as $posyandu)
                                    <option value="{{ $posyandu->posyandu_id }}"
                                        {{-- Isi otomatis dengan data lama atau data tersimpan --}}
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" name="tanggal"
                                {{-- Gunakan ->format() karena 'tanggal' adalah objek Carbon (dari $casts) --}}
                        ...
                        <div class="mb-3">
                            <label for="poster"
                                class="inline-block mb-2 ml-1 text-xs font-bold text-slate-700 dark:text-white/80">Ganti
                                Poster (Opsional)</label>

                            @if ($jadwal->poster)
                                <div class="my-2">
                                    <img src="{{ asset('storage/jadwal_posters/' . $jadwal->poster) }}" alt="Poster Saat Ini"
                            ...
                            <input type="file" name="poster"
                                class="focus:shadow-primary-outline dark:bg-slate-850 dark:text-white text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none">
                            <p class="text-xs text-slate-500 mt-1">Kosongkan jika tidak ingin mengubah poster.</p>
